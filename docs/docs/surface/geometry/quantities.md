...
??? func "transport vector across halfedge"
    ...
    - **require:** `void IntrinsicGeometryInterface::requireTransportVectorsAcrossHalfedge()`
    
    Example usage:
    <!--- IntrinsicGeometryInterface& geometry = *defaultGeomPtr; -->
    ```cpp
    ...
    for(Halfedge he : f.adjacentHalfedges()) {
      ...
      Vector2 rot = geometry.transportVectorsAcrossHalfedge[he];
...
??? func "transport vector along halfedge"
    ...
    for(Halfedge he : v.outgoingHalfedges()) {
      ...
      Vector2 rot = geometry.transportVectorsAlongHalfedge[he];
